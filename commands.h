...
inline void ParseRegister(const std::string& src, int32_t& dest, CPUMemory& memory) {
    if (string_to_register_number.find(src) != string_to_register_number.end()) {
        dest = memory.integer_registers[(*string_to_register_number.find(src)).second];
    } else {
...
inline void ParseRegister(const std::string& src, uint32_t& dest, CPUMemory& memory) {
    if (string_to_register_number.find(src) != string_to_register_number.end()) {
        dest = (uint32_t)memory.integer_registers[(*string_to_register_number.find(src)).second];
    } else {
        std::cerr << "preprocessor error: \"" << src << "\" is not a register\n";
        exit(1);
    }
...
public:
    void execute(const std::vector<std::string>& operands, CPUMemory& memory) override {
        ...
        int32_t& rd = memory.integer_registers[(*string_to_register_number.find(operands[0])).second];
        int32_t imm;
        ParseNumber(operands[1], imm);

        rd = imm << 12;

        memory.program_counter += kInstructionSize;
    }
...
public:
    void execute(const std::vector<std::string>& operands, CPUMemory& memory) override {
        ...
        int32_t& rd = memory.integer_registers[(*string_to_register_number.find(operands[0])).second];
        uint32_t& pc = memory.program_counter;
        int32_t imm;
        ParseNumber(operands[1], imm);
...
public:
    void execute(const std::vector<std::string>& operands, CPUMemory& memory) override {
        ...
        memory.LoadByte((*string_to_register_number.find(operands[0])).second, rs1 + offset);
        memory.program_counter += kInstructionSize;
    }
...
public:
    void execute(const std::vector<std::string>& operands, CPUMemory& memory) override {
        ...
        memory.LoadHalfWord((*string_to_register_number.find(operands[0])).second, rs1 + offset);
        memory.program_counter += kInstructionSize;
    }
...
public:
    void execute(const std::vector<std::string>& operands, CPUMemory& memory) override {
        ...
        memory.LoadWord((*string_to_register_number.find(operands[0])).second, rs1 + offset);
        memory.program_counter += kInstructionSize;
    }
...
public:
    void execute(const std::vector<std::string>& operands, CPUMemory& memory) override {
        ...
        memory.LoadUnorderedByte((*string_to_register_number.find(operands[0])).second, rs1 + offset);
        memory.program_counter += kInstructionSize;
    }
...
public:
    void execute(const std::vector<std::string>& operands, CPUMemory& memory) override {
        ...
        memory.LoadUnorderedHalfWord((*string_to_register_number.find(operands[0])).second, rs1 + offset);
        memory.program_counter += kInstructionSize;
    }
...
public:
    void execute(const std::vector<std::string>& operands, CPUMemory& memory) override {
        ...
        memory.StoreByte((*string_to_register_number.find(operands[0])).second, rs1 + offset);
        memory.program_counter += kInstructionSize;
    }
...
public:
    void execute(const std::vector<std::string>& operands, CPUMemory& memory) override {
        ...
        memory.StoreHalfWord((*string_to_register_number.find(operands[0])).second, rs1 + offset);
        memory.program_counter += kInstructionSize;
    }
...
public:
    void execute(const std::vector<std::string>& operands, CPUMemory& memory) override {
        ...
        memory.StoreWord((*string_to_register_number.find(operands[0])).second, rs1 + offset);
        memory.program_counter += kInstructionSize;
    }
